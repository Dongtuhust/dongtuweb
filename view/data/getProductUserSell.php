...
<div class="hover-effect" style="height: 50px">
	<h2 style="text-align: center; margin-top: 2em;">Sản phẩm do người dùng đăng bán</h2>
</div>
		...
		if(count($products) > 0){
			...
			while ($i < count($products))
			{
				...
				<tr valign="top">
					...
					<td style="max-height: 200px; max-width: 200px"><span><img class="card-img-top" src="<?=$row["product_img"]?>" alt="Card image"></span></td>
					<td><?=$row["product_name"]?></td>
					<td ><?=$row["price_buy"]?></td>
					<td ><?=$row["price_rent"]?></td>
					<td ><textarea class="form-control" rows="3" cols="100"><?=$row["description"]?></textarea></td>
...
<?php
 	echo getPaginationUnhref(count($totalProduct), $recordPerPage); 
?>